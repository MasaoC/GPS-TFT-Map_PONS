...
void draw_triangle(int ttrack,int steer_angle) {
  ...
  if (upward_mode == MODE_TRACKUP) {
    ...
    float tx1 = (NEEDLE_LEN-6) * sin(tone_left)   + BACKSCREEN_SIZE/2;
    float ty1 = (NEEDLE_LEN-6) * -cos(tone_left)  + BACKSCREEN_SIZE/2;
    float tx2 = (NEEDLE_LEN-6) * sin(tone_right)  + BACKSCREEN_SIZE/2;
    float ty2 = (NEEDLE_LEN-6) * -cos(tone_right) + BACKSCREEN_SIZE/2;
    float tx3 = NEEDLE_LEN     * sin(tone_center) + BACKSCREEN_SIZE/2;
    float ty3 = NEEDLE_LEN     * -cos(tone_center)+ BACKSCREEN_SIZE/2;
    if (trackwarning_until < millis()) {
    ...
    if ((millis()/1000)%3 != 0) {
      float arc_factor = course_warning_index / 900.0;
      if (steer_angle > 0)
        backscreen.drawArc(240/2, 240/2, (NEEDLE_LEN-21), (NEEDLE_LEN-23), 180, (180 + (int)(arc_factor*steer_angle)) % 360, COLOR_RED, COLOR_WHITE);
      else
        backscreen.drawArc(240/2, 240/2, (NEEDLE_LEN-21), (NEEDLE_LEN-23), (360 + 180 + (int)(arc_factor*steer_angle)) % 360, 180, COLOR_RED, COLOR_WHITE);
