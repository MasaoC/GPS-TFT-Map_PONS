...
// 生存確認（レジスタを 1 バイト読むだけ。電波は出さない）。
// mode 3 へ潜るので、受信中に呼ばないこと（呼び出し側が link_is_receiving() で除く）。
// 潜っている間は受信できない。所要は set_mode の AUX 待ちとボーレート切替を含めて
// 数十 ms。10 秒に 1 回しか呼ばない前提。
bool e220_ping();
