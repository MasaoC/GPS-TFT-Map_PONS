...
void imu_update() {
    ...
    // false が返るまで繰り返してキューを空にする（通常は 2 件 = GRV + LACC で終わる）。
    //
    // ※ 上限は「無限ループ防止の安全網」であって、通常の打ち切り手段ではない。
    //   このループは _qw.._laz を毎回上書きするため、KF predict に渡るのは最後の 1 件だけ。
    //   途中で打ち切ると「古いサンプル」が最新として使われ、
    //   time_us_32() から取る dt（下の Kalman predict）と時刻が食い違う。
    //
    //   旧実装の上限 10 では以下が起きていた:
    //     GRV 15Hz + LACC 15Hz + RV 5Hz = 35 件/秒。
    //     GPS ウォッチドッグの gps_setup() は Core0 を ~600ms ブロックするため
    //     約 21 件が BNO085 側のキューに溜まる。10 件で打ち切ると
    //     「10 件目 = まだ ~300ms 古いサンプル」を dt=600ms として積分してしまう。
    //     さらに残り 11 件が次のポーリングに持ち越され、
    //     300ms 分の運動を dt=30ms として積分する誤差が数サイクル尾を引く。
    //
    //   毎回キューを空にすれば生き残るのは常に最新サンプルとなり dt と整合する。
    //   上限 64 は dt > 0.5s 棄却（下記）の 500ms 分 = 約 18 件に対して十分な余裕がある。
    int read_count = 0;
    while (read_count < 64 && bno08x.getSensorEvent(&sv)) {
        read_count++;
        switch (sv.sensorId) {
