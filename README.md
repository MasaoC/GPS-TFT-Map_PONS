 ...
 * 最新のソフトウェアバージョンは **0.941**（Build 20260820）です。
...
# 機能 Detail.
 * 基本機能（画面の表示）
    * 画面上部：対地速度(m/s)、航路（TT=True Track）の表示。2.0m/s未満はグレー表示。
    * 画面中央：地図（ファームウェア内蔵のベクタ地図 ＋ SDの `mapdata.csv`）の表示、TTの針の表示。
      ...
      * 姿勢表示（BNO085搭載機のみ）：左下に上から `Avg30s P`（30秒平均ピッチ・大きい字）、
        `P`（瞬時ピッチ）、`R`（ロール）、`Rtrim`（自動ロールトリムの累積補正量）。
        フゴイドで瞬時ピッチは±3度振れるため、巡航のトリム状態は30秒平均で見ます。
        ロールがバンク角警告のしきい値（4度）を超えると赤くなります。
 ...
 * バリオメーター(MS5611)：昇降計。設定画面でボリューム0にすると無音・非表示。デッドバンドは±0.3m/s。BNO085 が無い個体は精度が不十分なため無効、β版のみ使用可。
 * 設定画面（NAV画面でスイッチ長押しで入る。各行の右端の丸⚫︎が設定状態を示す。
   **緑**＝本番推奨設定、**赤**＝本番では不可（デモ/リプレイが有効など）、**オレンジ**＝本番でも許容だが既定から外れている。
    ...
    * IMU / ESKF 画面（BNO085搭載機のみ。2ページ構成）
      * ESKF と BNO085 の3軸の比較、30秒平均ピッチ、自動ロールトリムの累積量
      * `SET PITCH` / `APPLY`：機体ゼロ点の較正。「いま機体のピッチは何度か」を申告して合わせます。
        本番プラットホーム（-3.5度）のような傾斜地でもその値を申告すれば正しく較正できます。
        **ロールは常に0として較正するので、機体が傾いた状態でAPPLYしないこと。** 飛行中は実行できません。
      * `Bank warning` / `Auto roll level detect` のON/OFF（既定はどちらもON）
      * 2ページ目：センサー生値とESKFの内部状態（診断用）
 ...
 * **バンク角警告**（BNO085搭載機のみ）：ロールの絶対値が4度を超えた状態が1秒以上続くと音声警報。
   ...
   * 同じ理由で **Auto roll level detect**（直進中のロール自動トリム）があります。
     直進が60秒続くごとに評価し、0.5度以上のズレがあれば0.5度ずつゼロへ寄せます（累積上限5度）。
...
# WAV音声ファイルの変更
 ...
 BNO085搭載機のみ: `bank_warning.wav`（バンク角警告）, `guide_eskf_setting.wav`（IMU/ESKF画面に入ったときの注意案内）, `eskf_apply_done.wav`（較正完了）
 ...
 * `imu_replaydata/`フォルダ（例：`imu_replaydata/20260324.txt`）：β版のみ、5Hz。
   `time, roll, pitch, yaw, pitch_avg, roll_trim, yaw_acc95`（ESKFの結果＝**画面に出ていた値そのもの**）
 ...
 * パイロンに向かいたい場合は、画面上の北パイロン・西パイロンへの線とTrue Track(TT)を見てパイロット自身が判断する。2025ルールではプラットホームから各パイロンへのTCはそれぞれ **318°** と **258°**（旧版に載っていた磁方位 326°/266° から偏角8°を引いた値）。
